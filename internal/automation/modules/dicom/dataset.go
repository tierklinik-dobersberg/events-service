...
func fromObject(v map[string]any) (*Dataset, error) {
	...
	for key, value := range v {
		if _, err := ds.Add(key, value); err != nil {
...
func (ds *Dataset) Add(name string, value any) (*Dataset, error) {
	t, err := findTag(name)
	if err != nil {
		return nil, fmt.Errorf("failed to find tag %s: %v", name, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to add element %s|%s: %v", name, t.String(), err)
	}
